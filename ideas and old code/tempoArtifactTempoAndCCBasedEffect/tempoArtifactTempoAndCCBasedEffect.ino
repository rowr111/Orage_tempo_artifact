...
void loop()
{
  if (half_revolutions >= 10)
  {
    ...
    { // send midi command to increase tempo
      if (tempo < 127)
      {
        //send midi command to increase volume of secondary effect
        if ((tempo > 62) && (volume < 126))
        {
          //volume has to get increased at twice the rate since it starts at halfway point
          volume = volume + 2;
    ...
    { // send midi command to decrease tempo
      if (tempo > 0)
      {
        //send midi command to decrease volume of secondary effect
        if (volume > 1)
        {
          //decreasing volume of secondary effect at twice the rate.
          volume = volume - 2;
          usbMIDI.sendControlChange(volume, velocity, 2);
        }
        tempo--;
